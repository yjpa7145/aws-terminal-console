...
const deleteStack = (StackName) => cf().deleteStack({ StackName }).promise();
...
const deleteStacks = async () => {
  ...
  const reallyDelete = await prompt({
    type: 'confirm',
    message: `Really delete ${pluralize(stacksToDelete.length, 'stack')} ${listToQuotedStrings(stacksToDelete)}?`,
    default: false
  });
...
const listStackResources = async () => {
  ...
  const response = await cf().listStackResources({ StackName }).promise();

  const resourceToLine = (r) => {
    const logicalResourceId = isNil(r.LogicalResourceId) ? '' : ` - ${r.LogicalResourceId}`;
    const physicalResourceId = isNil(r.PhysicalResourceId) ? '' : ` - ${r.PhysicalResourceId}`;
    const status = r.ResourceStatus === 'CREATE_COMPLETE' ? '' : ` - ${r.ResourceStatus}`;
...
const getStackEvents = async () => {
  ...
  let events = [];
  do {
    response = await cf().describeStackEvents({
      StackName,
  ...
  const eventToLine = (event) => {
    const physicalResourceId = R.isEmpty(event.PhysicalResourceId) ? '' : ` (${event.PhysicalResourceId})`;
    const reason = R.isNil(event.ResourceStatusReason) ? '' : `: ${event.ResourceStatusReason}`;

    return `${event.Timestamp.toISOString()} ${event.LogicalResourceId}${physicalResourceId} ${event.ResourceStatus}${reason}`;
  };
