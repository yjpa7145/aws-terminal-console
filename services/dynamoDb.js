...
const deleteTable = (TableName) =>
  dynamo().deleteTable({ TableName }).promise();

const getTableNames = async () => {
  let response = { LastEvaluatedTableName: null };
  let names = [];
  do {
    response = await dynamo().listTables({ ExclusiveStartTableName: response.LastEvaluatedTableName }).promise();
...
const deleteTables = async () => {
  ...
  const confirmationMessage = `Really delete ${pluralize(tableNames.length, 'table')} ${listToQuotedStrings(tableNames)}?`;
